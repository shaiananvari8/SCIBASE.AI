...
fs.writeFileSync(path.join(reportsDir, "webhook-replay-packet.json"), `${JSON.stringify(reviewerPacket, null, 2)}\n`);
fs.writeFileSync(path.join(reportsDir, "webhook-replay-report.md"), renderMarkdownReport(packet, { now: packet.generatedAt }));
...
console.log(JSON.stringify({
  ...
  heldEvents: reviewerPacket.evaluation.counts.heldEvents,
  reportsDir
}, null, 2));
